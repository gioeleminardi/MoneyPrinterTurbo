...
def _generate_response(prompt: str) -> str:
    """Generate all application text through AIHubMix."""
    try:
        api_key = config.app.get("aihubmix_api_key")
        if not api_key:
            raise ValueError(
                "aihubmix_api_key is not set, please set it in the config.toml file."
            )
        model_name = config.app.get("aihubmix_model_name") or "gpt-5.4-mini"
        base_url = config.app.get("aihubmix_base_url") or "https://aihubmix.com/v1"
        logger.info(f"requesting AIHubMix chat completion, model: {model_name}")
        ...
        try:
            ...
            client.close()
        return _extract_chat_completion_text(response, "aihubmix")
